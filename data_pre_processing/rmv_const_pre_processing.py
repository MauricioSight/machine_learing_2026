...
class RmvConstPreProcessing(DataPrePrecessing):
    def initialize(self, data_loader: DataLoader) -> Tuple[np.ndarray, pd.DataFrame]:
        data, target = data_loader.load()
        return self.process(data, target)

    def process(
        self, data: np.ndarray, target: pd.DataFrame
    ...
    def load(self, path: str) -> tuple[np.ndarray, pd.DataFrame]:
        ...
        if load_subset is not None:
            ...
            indices = np.random.choice(
                len(X), size=int(load_subset * len(X)), replace=False
            )
            X = X[indices]
