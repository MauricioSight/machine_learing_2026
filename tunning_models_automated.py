...
def main(tune_run_id: str = None, config_file="tune-mlp"):
    ...
    train_config = load_config(
        config_name=tune_config["train_config"], default_file_name="train_config"
    )
    ...
    if "run_id" not in tune_config:
        run_id = get_tune_id(
            ...
            train_config,
            [
                tune_config["framework"],
                tune_config["algorithm"],
    ...
    def objective_fn(params):
        ...
        experiment_id = get_run_id(
            updated_config,
            [
                updated_config["modeling"]["structure"]["name"],
                updated_config["data_loader"]["name"],
            ],
        )
