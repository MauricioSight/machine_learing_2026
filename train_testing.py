...
def main(config=None, X=None, y_true=None):
    ...
    train_out, test_out = trainer.train(model, X, y_true, metrics_handler)
    ...
    save_run_artifacts(
        run_dir, {**config, "phase": "train_testing_train"}, metrics=train_metrics
    )
    save_run_artifacts(
        run_dir, {**config, "phase": "train_testing_test"}, metrics=test_metrics
    )
